...
var MonitorServer = class {
  ...
  setServerIntervals(opts) {
    ...
    this.presenceCheckMs = clamp(opts.presenceCheckMs, 5e3, 3e5, this.presenceCheckMs);
    this.syncScanMs = clamp(opts.syncScanMs, 5e3, 3e5, this.syncScanMs);
    this.serverPresenceMs = clamp(opts.serverPresenceMs, 5e3, 3e5, this.serverPresenceMs);
    if (this.running) {
      this.fallback.startPolling(this.backlogPollMs);
      if (this.syncScanInterval) {
        clearInterval(this.syncScanInterval);
  ...
  async setClientPollInterval(clientKey, intervalMs) {
    this.clientPollMs = Math.max(3e3, Math.min(3e5, intervalMs));
  ...
  async setClientUpdateCheckInterval(clientKey, intervalMs) {
